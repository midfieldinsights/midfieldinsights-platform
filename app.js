...
class MarketResearchPlatform {
    constructor() {
        ...
        this.projects = [
            {
                ...
                prescreenerQuestions: [
                    ...
                    {
                        id: 3,
                        question: "Which electronics did you purchase? (Select all that apply)",
                        type: "multiple_choice",
                        options: ["Smartphone", "Laptop", "Tablet", "Smart TV", "Gaming Console", "Smart Watch", "Other"],
                        qualifyingAnswers: ["Smartphone", "Laptop", "Tablet", "Smart TV"]
                    }
                ]
            },
            {
                ...
                deadline: "2025-10-15",
                surveyLink: "https://surveys.midfieldinsights.com/healthcare-study",
                redirectUrls: {},
                vendorsAssigned: [],
                prescreenerQuestions: []
            }
        ...
        this.vendors = [
            ...
            },
            {
                ...
                contactEmail: "hello@globalsurv.com",
            ...
            if (project.status === 'active' && project.surveyLink) {
                ...
                surveyCard.innerHTML = `
                    ...
                    <div style="margin-top: 15px;">
                        ...
                        <button class="btn btn-secondary btn-small" onclick="app.configureRedirects(${project.id})">Configure Redirects</button>
                    </div>
                `;
    ...
    createProject() {
        const formData = {
            ...
            prescreenerQuestions: [],
            redirectUrls: {
                complete: `https://platform.midfieldinsights.com/complete?pid=${this.projects.length + 1}&clickid={CLICK_ID}&status=complete`,
                screenout: `https://platform.midfieldinsights.com/screenout?pid=${this.projects.length + 1}&clickid={CLICK_ID}&status=screenout`,
                quotaFull: `https://platform.midfieldinsights.com/quota?pid=${this.projects.length + 1}&clickid={CLICK_ID}&status=quota_full`,
                incomplete: `https://platform.midfieldinsights.com/incomplete?pid=${this.projects.length + 1}&clickid={CLICK_ID}&status=incomplete`
            }
        };
        ...
        alert('Project created successfully! Redirect URLs have been automatically generated.');
    }
    ...
    addVendor() {
        const formData = {
            ...
            panelSize: parseInt(document.getElementById('vendor-panel-size').value) || 0,
            rating: 0,
            completionRate: 0,
            status: 'active'
        };
    ...
    launchSurvey(projectId) {
        ...
        const availableVendors = this.vendors.filter(v => v.status === 'active');
        let vendorOptions = availableVendors.map(v => `${v.name} (${v.specialty})`).join('\\n');

        alert(`Survey Launch Management for "${project.title}"\\n\\nAvailable Actions:\\n• Assign vendors from database\\n• Send manual invitations\\n• Configure launch parameters\\n• Set quotas and targeting\\n• Monitor real-time progress\\n\\nAvailable Vendors:\\n${vendorOptions}`);
    }
    ...
    managePrescreeners(projectId) {
        ...
        alert(`Pre-screener Management for "${project.title}"\\n\\nFeatures Available:\\n• Add/edit screening questions\\n• Set qualification logic and skip patterns\\n• Configure demographic quotas\\n• Test question flow\\n• Preview respondent experience\\n\\nCurrent Questions: ${project.prescreenerQuestions.length}`);
    }

    configureRedirects(projectId) {
        const project = this.projects.find(p => p.id === projectId);
        if (!project) return;

        let redirectInfo = Object.entries(project.redirectUrls).map(([type, url]) => `${type}: ${url}`).join('\\n');

        alert(`Redirect URL Configuration for "${project.title}"\\n\\nCurrent Redirect URLs:\\n${redirectInfo}\\n\\nFeatures:\\n• Customize redirect URLs\\n• Add tracking parameters\\n• Test redirect flows\\n• Configure S2S callbacks`);
    }
    ...
    assignVendor(vendorId) {
        ...
        const activeProjects = this.projects.filter(p => p.status === 'active' || p.status === 'bidding');
        let projectOptions = activeProjects.map(p => `${p.title} (${p.status})`).join('\\n');
...
document.addEventListener('DOMContentLoaded', function() {
    window.app = new MarketResearchPlatform();
});